...
*/
public class Day9SensorBoostPart1 {
	...
	public static void main(String args[]) throws Exception {
		Day9SensorBoostPart1 day9SensorBoostPart1 = new Day9SensorBoostPart1();
		...
		private void extractAndExecuteOptCodes(long intCodes[], int startPosition) {
			...
			if (OPT_CODE_99 == optCode) {
				...
				if (firstParmeterMode == POSITION_MODE ) {
					firstParamValue = intCodes[(int) intCodes[startPosition + 1]];
				} else if (firstParmeterMode == IMMEDIATE_MODE) {
					firstParamValue = intCodes[startPosition + 1];
				} else if (firstParmeterMode == RELATIVE_MODE) {
					int value = (int) intCodes[startPosition + 1];
					firstParamValue = intCodes[value + relativeBase];
				}
				
				if (secondParmeterMode == POSITION_MODE) {
					secondParamValue = intCodes[(int) intCodes[startPosition + 2]];
				} else if (secondParmeterMode == IMMEDIATE_MODE) {
					secondParamValue =intCodes[startPosition + 2];
				} else if (secondParmeterMode == RELATIVE_MODE) {					
					int value = (int) intCodes[startPosition + 2];
					secondParamValue = intCodes[value + relativeBase];

				}
				
				long sum = firstParamValue + secondParamValue;
				if (thirdParmeterMode == POSITION_MODE || firstParmeterMode == IMMEDIATE_MODE) {
					intCodes[(int) intCodes[startPosition + 3]] = sum;
				} 
//				else if (thirdParmeterMode == IMMEDIATE_MODE) {
//					intCodes[startPosition + 3] = sum;
//				}  
				else {
					// intCodes[(int) intCodes[startPosition + 3] + relativeBase] = sum;
					int value = (int) intCodes[startPosition + 3];
					intCodes[value + relativeBase] = sum;
				}
				...
				if (thirdParmeterMode == POSITION_MODE || firstParmeterMode == IMMEDIATE_MODE) {
					intCodes[(int) intCodes[startPosition + 3]] = product;
				} 
//				else if (thirdParmeterMode == IMMEDIATE_MODE) {
...
//				}  
					...
					// intCodes[(int) intCodes[startPosition + 3] + relativeBase] = sum;
					int value = (int) intCodes[startPosition + 3];
					intCodes[value + relativeBase] = product;
				}
				increamentFactor = startPosition + 4;
...
//				} 
					...
					int value = (int) intCodes[startPosition + 1];
					intCodes[value + relativeBase] = input;
				}
...
//				}
				...
				long thirdParmValue;
				if (thirdParmeterMode == POSITION_MODE || thirdParmeterMode == IMMEDIATE_MODE) {
					thirdParmValue = intCodes[(int) intCodes[startPosition + 3]];
				} 
//				else if (thirdParmeterMode == IMMEDIATE_MODE) {
//					thirdParmValue = intCodes[startPosition + 3];
//				} 
				else {
					int value = (int) intCodes[startPosition + 3];
					thirdParmValue = intCodes[value + relativeBase];
				}
				
				if (firstParamValue < secondParamValue) {
					intCodes[(int) thirdParmValue] = 1;
				} else {
					intCodes[(int) thirdParmValue] = 0;
				}
				increamentFactor = startPosition + 4;
...
//				} 
					...
					// thirdParmValue = intCodes[(int) intCodes[startPosition + 3] + relativeBase];
					int value = (int) intCodes[startPosition + 3];
					thirdParmValue = intCodes[value + relativeBase];
				}
...
//		public setParamsValue(int sourceIndex, long value ) {
//			if (sourceIndex >= intCodes.length) {
//				
//			} else {
//				intCodes[sourceIndex] = 0;
//			}
//		}
	}
