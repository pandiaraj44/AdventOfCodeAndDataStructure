...
*/
public class Day13CarePackagePart1 implements AOC<Long[][]>{
...
//				intCodeComputer.inputValues.add(colorMap.getOrDefault(key, 0L));
			...
			if (haltCode == HALT_CODE_OUTPUT && intCodeComputer.outputList.size() == 3) {				
				...
				String mapKey = distanceFromLeft + "," + distanceFromTop;
				tileMap.put(mapKey, tileId);
				
				intCodeComputer.outputList.clear();
			}
		}
		
		System.out.println("Grid size---"+ tileMap.size());
		
		
		
		grid = constructGrid(tileMap);
		int blockCount = 0;
		for (Long[] gridItems: grid) {
			for (long item : gridItems) {
				if (item == TILE_BLOCK) {
					blockCount++;
				}
			}
		...
		System.out.println("Block size---"+ blockCount);
		return grid;
	}
	...
	static class IntCodeComputer {
		...
		private void extractAndExecuteOptCodes() {
			while(!Day13CarePackagePart1.HALT_CODES.contains(this.haltCode)) {
