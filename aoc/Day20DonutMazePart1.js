...
const sourcePoint = points['AA'][0];
const destinationPoint = points['ZZ'][0];
...
function processInput(input) {
  ...
  for (let pointValues of Object.values(points)) {
    ...
    const [gate1, gate2] = pointValues;
    gateways[gate1.join(',')] = gate2;
    gateways[gate2.join(',')] = gate1;
  }
...
function getAdujacentPoints([x,y]) {
  const result = [];
  ...
  if (gateways[getKey([x,y])]) {
    result.push(gateways[getKey([x,y])]);
  }
  return result;
}
